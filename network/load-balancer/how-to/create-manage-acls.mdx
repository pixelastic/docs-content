...
4. Click the name of the frontend you want to create ACLs for.
  ...
  The **ACLs** tab displays. If you have any existing ACLs they are listed here, otherwise you see a splashscreen.

  <Lightbox src="scaleway-acl-splash.webp" alt="" />

5. Click **Create an ACL**. The ACL creation wizard displays.
...
7. Select an **action** from the following options:
...
8. Use the **Enable condition** toggle to choose whether to set a condition for the action.
...
9. Set the condition type for your action, if you enabled a condition in the previous step. Choose from:
...
10. Add filters, if you enabled a condition at step 8. Traffic must match (or not match, in the case of the IF NOT condition) the filters in order for the action to be applied. You can select one type of filter per ACL, or combine one IP filter and one HTTP header filter with an **AND** statement.
...
11. Click **Create ACL** to finish. Your ACL is created, and you are redirected to the ACLs tab, where your new ACL now appears.

12. Repeat steps 5 to 11 to create new ACLs as required. Remember that they will be applied in order, according to the priority number you give each ACL.
    <Message type="tip">
...
3. Click the **Frontends** tab.
4. Click the name of the frontend frontend whose ACL(s) you want to edit. You are taken to the ACL tab for that frontend. The list of your ACLs displays.
...
3. Click the **Frontends** tab.
4. Click the name of the frontend frontend whose ACL(s) you want to edit. You are taken to the ACL tab for that frontend. The list of your ACLs displays.
5. Click the <Icon name="more" /> icon next to the ACL you wish to delete, and select **Delete**.
